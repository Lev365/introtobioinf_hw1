...
```{}
Type your answer here:
H_0: gender has no effect on death rate due to covid.
```

- What is the p-value? Is it significant (alpha = 0.05)? Write your conclusions from the statistical test results.
```{}
Type your answer here:
p-value is the probability to get data at least as extreme as the data we got, given H_0 is true.
the p-value is 0.077, higher than alpha = 0.05, thus we accept H_0
```
...
```{}
My null hypothesis:
H_0: age has a positive correlation with death rate due to covid.
My alternative hypothesis:
age has no effect on death rate due to covid.
```
